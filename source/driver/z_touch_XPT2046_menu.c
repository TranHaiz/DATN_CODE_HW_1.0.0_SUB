...
void DrawMenuItem(sMenuItem *item, uint8_t selected)
{
  ...
  Displ_CString(item->X, item->Y, item->X + item->W - 1, item->Y + item->H - 1, item->Desc, item->font, item->fontSize,
                ink, bkg);
...
uint8_t CheckMenu(sMenuItem *menu, uint8_t menusize)
{
  ...
  for (k = 0; k < menusize; k++)
  {
    if (Touch_In_XY_area(menu[k].X, menu[k].Y, menu[k].W, menu[k].H))
    {  // polling if touch was in the current menu item area
      result = k;
      break;
    }
  }
