...
volatile uint8_t Touch_Int_Enabled =
  1;  // while reading touch sensor touch interrupt handling is disabled through this flag
...
uint8_t Touch_PollTouch()
{
  ...
  touch = 0;
  for (k = 0; k < (1 << pollingLevel); k++) touch += Touch_PollAxis(Z_AXIS);
...
void Touch_GetXYtouch(uint16_t *x, uint16_t *y, uint8_t *isTouch)
{
  ...
  // reading X
  touch = 0;
  for (k = 0; k < (1 << pollingLevel); k++) touch += Touch_PollAxis(X_AXIS);
  touch >>= pollingLevel;  // get the average value
  ...
  touch = 0;
  for (k = 0; k < (1 << pollingLevel); k++) touch += Touch_PollAxis(Y_AXIS);
  touch >>= pollingLevel;  // get the average value
...
uint8_t Touch_WaitForUntouch(uint16_t delay)
{
  ...
  while (1)
  {
    ...
    if (Touch_PollAxis(X_AXIS) <= X_THRESHOLD)
      pen_up = 1;
    if (pen_up)
    {                 //	Pen is now up: reset Touch_PenDown anyway.
...
uint8_t Touch_TouchGFXSampleTouch(int32_t *x, int32_t *y)
{
  ...
  static uint8_t flipTouch =
    0;  // switches 0/1, on every function call, until sensor is touched allowing to return key repeat
  static uint32_t touchTime = 1;  // tick value get on the first touch. 0 means display untouched.
  static uint16_t avgXX     = 0,
                  avgYY     = 0;  // need to convert library coordinates type (uint16_t) to TouchGFX one (int32_t)
  static uint8_t repetition = TOUCHGFX_REPEAT_IT + TOUCHGFX_REPEAT_NO;

  if (Touch_GotATouch(0))
  {                                        // polls interrupt flag not resetting it
    Touch_GetXYtouch(&xx, &yy, &isTouch);  // get touch sensor position
    if (!isTouch)
    {  // received a "no touch"
      if (touchTime != 0)
      {  // if previously touched
        if ((repetition--) > TOUCHGFX_REPEAT_NO + 1)
        {  // n-repetition of last touch sending
          *x      = avgXX;
          *y      = avgYY;
          isTouch = 1;
        }
        else if ((repetition == 255))
        {                                                        // that's -1
    ...
    {  // display touched
      ...
      {  // not a new touch
        if (((HAL_GetTick() - touchTime) > DELAY_TO_KEY_REPEAT) && (DELAY_TO_KEY_REPEAT > 0))
        {                          // if timeout to key repeat is over (0 means no key repeat)
          flipTouch = !flipTouch;  // alternate every time function is called
        }
        else if (DELAY_TO_KEY_REPEAT == 0)
          flipTouch = 0;  // (DELAY_TO_KEY_REPEAT == 0) means a single pulse, "-1" keep pulse as long as touch
      }
      if (flipTouch)
      {  // return position only if the switching flag is on
        *x =
          (((avgXX * (TOUCHGFX_MOVAVG - 1) + ((xx / TOUCHGFX_SENSITIVITY) * TOUCHGFX_SENSITIVITY))) / TOUCHGFX_MOVAVG);
        *y =
          (((avgYY * (TOUCHGFX_MOVAVG - 1) + ((yy / TOUCHGFX_SENSITIVITY) * TOUCHGFX_SENSITIVITY))) / TOUCHGFX_MOVAVG);
        avgXX = *x;
        avgYY = *y;
      }
      else
      {  // otherwise return "no touch" from display
        isTouch = 0;
      }
    }
  }
