...
static uint8_t          sd_initialized = false;
...
)
{
  ...
  uint8_t sd_type = sd_card_init();
  if (sd_type != SD_TYPE_ERR)
  {
    sd_initialized = 1;
    return 0;  // Success - return 0 (no flags set)
  }
  sd_initialized = 0;
  return STA_NOINIT;
  /* USER CODE END INIT */
...
)
{
  ...
  // Use blocking read (non-DMA) for stability
...
)
{
  ...
  // Use blocking write (non-DMA) for stability
  for (UINT i = 0; i < count; i++)
  {
    if (sd_card_write_block((uint8_t *) (buff + (i * 512)), sector + i) != 0)
      return RES_ERROR;
  }
  return RES_OK;
  /* USER CODE END WRITE */
...
)
{
  ...
  if (!sd_initialized && cmd != CTRL_SYNC)
    return RES_NOTRDY;
  ...
  case GET_SECTOR_COUNT:
    *(DWORD *) buff = sd_card_get_sector_count();
    res             = RES_OK;
    break;
